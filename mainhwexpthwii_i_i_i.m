...
clear
%close all
%When simulating different policies make sure to change
%n_b, state0(nst,:), hwmult, bet, 
...
atstop = ones(1,n_b)*0; %These are the flags which will be 1
...
lapass = zeros(1,n_b);
lpass = zeros(1,n_s);
w_pass = zeros(nb_start,n_s); %to store the number of walking passengers to a particular stop
%at max there can be 4 types of passengers walking towards a particular
%stop
tw_pass = zeros(nb_start,n_s); %time to reach the next stop
unit_cap = 100;
...
%hwmult = 1; %Headway multiplier to switch between no control and other policies
scamcount = 0;
...
%hwt_i = 264;
hwt = hwt_i*hwmult;
hwt2 = (sum(dis_stp)/v_bus + n_s*fixdt)/(2 - (t_al + t_bo)*arrmu*n_s);
hwt4 = (sum(dis_stp)/v_bus + n_s*fixdt)/(4 - (t_al + t_bo)*arrmu*n_s);
cumtime = zeros(1,n_b);
splittime = 1; %splittime being 1 second -- Introducing this constant to avoid bus overtake
stload = floor(arrmu*n_s/2*hwt_i); 
%stload = 55;
state0(2,:) = stload*ones(1,n_b);
...
for i=1:n_b 
    t_nxt(i) = hwt_i*(i-1) + dis_stp(n_s)/v_bus;
end
%disp(t_nxt)
l_action = zeros(n_st,n_b); %last action taken by th bus
papbcum = 0;
papbskip = 0;
count = 0; %This will account the number of rounds
roundcomp = false; %flag to track whether first module completed a round or not
...
%         end
        ...
        if state(3,im) == 1 %If the bus is joined then the split action will always have highest reward as we are using the AVERAGE in rewards
            imr = 3; %imr value corresponding to the split action
            action  = 'split';
            spcount = spcount +1;
...
%             end
            ...
            for bus = 1:n_b %this takes care of overtaking
                if state(1,bus) == state(1,im) && atstop(im) == 0
                    if t_nxt_i(bus) > mxtraveltime
                        mxtraveltime = t_nxt(bus);
                    end
                end
            end
            ct = max(mxtraveltime, ct) +1; %adding 1 to avoid two buses reaching at the same time
    ...
    %
                ...
                if ex_wt ~=0
                    atstop(im) = 1;
                    t_nxt(im) = ex_wt + 1; %adding 1 to ensure that the module reaching late departs late
                else
                    ct = dis_stp(state(1,im))/v_bus + gamrnd(gam_k, gam_theta) - gam_theta*gam_k;  %this tspent can be the t_nxt two lines above
                    mxtraveltime = 0; %this takes care of overtaking
                    for bus = 1:n_b
                    ...
                    ct = max(mxtraveltime, ct) + 1; %adding 1  to ensure module leaving late reaches late
                    t_nxt(im) = ct + ex_wt;
                end
            end
            ...
            if joincond
                ...
                if im == n_b
                   ...
                   % pw = binornd((load_rj-lapass(1)),a_par(state(1,n_b)));
                    ...
                    %updating lapass
                    %pw = binornd((load_rj-lapass(implus)),a_par(state(1,implus)));
                    lapass_i = zeros(1,n_b);
                    for i=1:im-1
                        lapass_i(i) = lapass(i);
                    end
        ...
        %If join is not feasible  
                ...
                imr = 5;
                action = 'nextbs';
                ct = dis_stp(state(1,im))/v_bus + gamrnd(gam_k, gam_theta) - gam_theta*gam_k;
                mxtraveltime = 0; %this takes care of overtaking
                for bus = 1:n_b
                    if state(1,n_b) == state(1,im) && atstop(im) == 0
                        if t_nxt(n_b) > mxtraveltime
                            mxtraveltime = t_nxt(bus);
                        end
                    end
                end
                ct = max(mxtraveltime, ct);
                tspent = ct;  %this tspent can be the t_nxt two lines above
                t_nxt(im) = tspent;
                atstop(im) = 0;
            end          
    end
    
    if time < simtime*3600 && time > 0
...
%     Skipcum = [Skipcum skcount];
%     
%     %currrently incorrect need to modify this
...
Pa_pre_cum_int = trapz(Pa_pre_cum(ceil(stime):ceil(e_time)));
...
avg_wait1 = (Pa_pre_cum_int - Pb_cum_int)/(Pa_pre_cum(ceil(e_time)) - Pa_pre_cum(ceil(stime)));
...
[~,j1] = min(abs(e_time - Time1));
[~,i2] = min(abs(stime - Time2));
[~,j2] = min(abs(e_time - Time2));
[~,i3] = min(abs(stime - Time3));
[~,j3] = min(abs(e_time - Time3));
[~,i4] = min(abs(stime - Time4));
[~,j4] = min(abs(e_time - Time4));
...
% hold on 
% plot(Time_pre(ceil(stime):ceil(e_time)), Pa_pre_cum(ceil(stime):ceil(e_time)))
