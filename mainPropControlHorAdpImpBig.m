...
n_b = 12; %Number of bus, 1 bus = 2 modules modules starts in the detached state
nb_start = n_b;
tot_mod = nb_start;
...
hor = 1 ;
fPas1 = 3.5; fPas2 =6.5; %reward function parameters %0.6, 0.7 best for hor =1, seed = 3.
...
%         end
    ...
    %             end
                ...
                stopTime = t_al*pdsr + t_bo*pbsr + fixdt + ex_wt + splittime;
                tspent = stopTime;
...
%
                ...
                stopTime = t_al*pdsr + t_bo*pbsr + fixdt + ex_wt;
                stopTimeA = stopTimeA + stopTime;
                tspent = stopTime;
...
%         disp(t_nxt)
        ...
        opt_act = str2num(opt_act(1));
        imr = opt_act + 1;
%         if state(1,im) == state(1,implus) && atstop(implus) == 1 && t_nxt(implus) < 30 && state(3,im) == 0 && state(3,implus) == 0
%             disp(t_nxt)
%             imr = 4;
%         end
%         if gencount == 541 
...
%          if gencount > 2370 && gencount < 2375
...
for i = 1: tot_mod
    for j = 1:size(ilist{i},2)-1
        ...
        plot(mod_time{i}(i1:i2), locs{i}(i1:i2), 'Color', colA(i))
    end
end
yticks(1:n_s)
grid on
% plot(mod_time{1}, locs{1})
% hold on
% for i=2:tot_mod
%     plot(mod_time{i}, locs{i})
% end
% figure(1)
% plot(Time1, loc1)
% xlabel('time (seconds)')
