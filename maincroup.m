...
%     end
    ...
    %     end
        ...
        r_st = Rewardu(im, state, 0, a_par,arr_par, dis_stp, v_pas, ...
        hw,unit_cap,lpass(im),l_action(im), lapass(im), hw(implus), t_bo, t_al);
    
        r_sk = Rewardu(im, state, 1, a_par,arr_par, dis_stp, v_pas, ...
        hw,unit_cap,lpass(im),l_action(im), lapass(im), hw(implus), t_bo, t_al);
    
        r_sp = Rewardu(im, state, 2, a_par,arr_par, dis_stp, v_pas, ...
        hw,unit_cap,lpass(im),l_action(im), lapass(im), hw(implus), t_bo, t_al);
       % r_st = 1; r_sk = 0;
%         fprintf('Stop rew %f \n', r_st)
%         fprintf('Split rew %f \n', r_sp)
...
%             end
            ...
            %function as it is more accurate. No need to update headway
            %here
%             hw_i = zeros(1,n_b);
%             for i = 1:im
%                 hw_i(i) = hw(i);
...
%         end
        if state(3,im) == 0 && state(3,implus) == 0 %Join have positive reward and it is indeed possible to join
            r_jn = Rewardu(im, state, 3, a_par,arr_par, dis_stp, v_pas, ...
                   hw,unit_cap,lpass(im),l_action(im), lapass(im), hw(implus), t_bo, t_al);
        else
