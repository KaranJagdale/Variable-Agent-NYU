...
for simn = 1:nsim
    ...
    while true
        ...
        loadPM = loadPM + sum(state(2,:))/tot_mod;
        fb = 0;
        for i=1:size(state,2)
            if state(3,i) == 1 && state(2,i) == 100
                fb = fb + 1;
            end
            if state(3,i) == 0 && state(2,i) == 50
                fb = fb + 1;
            end
        end
        ffbAve = ffbAve + fb;
    ...
    %     end
        ...
        if stimeRec
            if time > stime + 500 && locs{1}(end) == inSt && ~cycTRec 
                cycT = (time - stime)/60;
                cycTRec = true;
            end
        end
    ...
    cycTAve = cycTAve + cycT;
    loadAve = loadAve + loadPM/(gencount -1);
    ffbAveA = ffbAveA + ffbAve/(gencount - 1);
    i = 1;
    while Time(i) < etime
        i = i+1;    
    end
    ...
    costWalk(simn) = avg_walk1/60;
    costWait(simn) = avg_wait1/60;
    costInV(simn) = avg_inveh/60;
end

fprintf('average travel time obtained : %f \n', mean(costHor1))
fprintf('average walking time : %f\n', mean(costWalk))
...
fprintf('average loasd : %f \n', loadAve/nsim)
fprintf('average cycle time : %f \n', cycTAve/nsim)
fprintf('average fraction of full buses : %f \n', ffbAveA/nsim)