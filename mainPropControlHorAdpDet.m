...
%     end
    ...
    if atstop(im) == 0
       ...
       % hway = hw(im);
        ...
        opt_act = rewsum_t(hor, state, t_nxt_sim, atstop, t_al, t_bo, arr_par, a_par, dis_stp, im, hway, v_bus, v_pas, lpass, lapass, cap_bus, hwt,ord, count, gencount, reachtime, leavetime, time,Hway);
...
%         disp(Hway)
        ...
        imr = opt_act + 1;
        rewo = Reward_t(im, state, opt_act, a_par,arr_par, dis_stp, v_pas, ...
        hw(im),cap_bus,lpass(state(1,im)),0, lapass(im), hw(implus), t_bo, t_al,hw(implusp), hwt, count,v_bus, gencount, 0, 0); %passing 0 for pos_code and actn as these values dont  matter here
        if gencount > rewcsind % && gencount < rewcsind + 5
            rewc = rewc + rewo;
