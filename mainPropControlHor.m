...
%successively
%dbstop if naninf
simtime = 30; %simulation time in hours
...
%         disp(size(Time,2))
    ...
    if atstop(im) == 0
       ...
       % hway = hw(im);
        ...
        opt_act = rewsum_t(hor, state, t_nxt_sim, atstop, t_al, t_bo, arr_par, a_par, dis_stp, im, hway, v_bus, v_pas, lpass, lapass, cap_bus, hwt,ord, count, gencount);
...
%         disp(t_nxt)
        ...
        hw(im),cap_bus,lpass(im),0, lapass(im), hw(implus), t_bo, t_al,hw(implusp), hwt);
        rewc = rewc + rewo;
        ...
        joincond =  r_jn > 0  && ((atstop(implus) == 0 && state(1,implus) ~= state(1, im)) || state(1, implus) == state(1,im)); 
            if joincond
                ...
                for st = stop_rj:state(1,im)
                    lap = binornd(load_rj - lapass(implus) - lap_ac,a_par(st));
                    lap_ac = lap_ac + lap;
                end
                if im == n_b                    
                    n_b = n_b -1;
