...
%     end
    ...
    rskip = -pds*wst - pbs*phead + (state(2)-pds)*(pds*t_al + pbs*t_bo) + hwg*(hw - hwt);% + pbsplus*(pds*t_al + pbs*t_bo);
    rstop = -rskip;
    rsplit = (state(2)-pds)*(pds*t_al + pbs*t_bo) +pds*wst +pbs*phead + hwg*(hw - hwt); % pbsplus*(pds*t_al + pbs*t_bo);
...
%     fprintf('10*(hw - hwt) = %f \n', 10*(hw - hwt))
    ...
    if action == 3        
        ...
        for i=stbkbus
            ...
            wst = dis_stp(nxt_stp)/v_pas;
            rjskip = -p_ds*wst - p_bs*pphead + (stateb(2)-p_ds)*(p_ds*t_al + p_bs*t_bo);% + 10*(phead - hwt);
        ...
        rskipp = -pdsplus*wstp - pbsplus*phead + (state(2)-pdsplus)*(pdsplus*t_al + pbsplus*t_bo) + hwg*(hw - hwt);
        rsplitp = (state(2)-pdsplus)*(pdsplus*t_al + pbsplus*t_bo) +pdsplus*wst +pbsplus*phead + hwg*(hw - hwt);
        R1 = rsplitp;
        R2 = abs(rskipp);
        rjoin = R1- R2  - phead*state(2) + rjskipc;
    end
