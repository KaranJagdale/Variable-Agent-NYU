...
%         disp(size(Time,2))
    ...
    if atstop(im) == 0
       ...
       % hway = hw(im);
        ...
        r_st = Rewarduhwi(im, state, 0, a_par,arr_par, dis_stp, v_pas, ...
        hway,cap_bus,lpass(im),l_action(im), lapass(im), hw(implus), t_bo, t_al,hw(implusp), hwt);
    
        r_sk = Rewarduhwi(im, state, 1, a_par,arr_par, dis_stp, v_pas, ...
        hway,cap_bus,lpass(im),l_action(im), lapass(im), hw(implus), t_bo, t_al, hw(implusp), hwt);
    
        r_sp = Rewarduhwi(im, state, 2, a_par,arr_par, dis_stp, v_pas, ...
        hway,cap_bus,lpass(im),l_action(im), lapass(im), hw(implus), t_bo, t_al, hw(implusp), hwt);
        streachct = streachct + 1;
        if r_sk > r_st
            skgstct = skgstct + 1;
        end
        rew_arr = [r_st, r_sk, r_sp];
        [m, imr] = max(rew_arr);
        %imr = 1;
        %disp(imr)
...
%         end
        if state(3,im) == 0 && state(3,implus) == 0 %Join have positive reward and it is indeed possible to join
            r_jn = Rewarduhwi(im, state, 3, a_par,arr_par, dis_stp, v_pas, ...
                   hway,unit_cap,lpass(im),l_action(im), lapass(im), hw(implus), t_bo, t_al, hw(implusp), hwt);
        else
