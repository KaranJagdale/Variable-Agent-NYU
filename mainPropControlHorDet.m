...
%successively
%dbstop if naninf
simtime = 20; %simulation time in hours
...
%     disp(gencount)
    if floor(pe_cum) ~= pe_cum
        fprintf('pe_cum is no more int, pe_cum : %f \n', pe_cum)
        break
    end
...
%         disp(size(Time,2))
    ...
    if pd_cum < pe_cum
        fprintf('issue in start, pd_cum : %f, pe_cum : %f \n', pd_cum, pe_cum)
        break
    end
...
%         disp('actual state')
%         disp(state)
        opt_act = rewsum(hor, state, t_nxt, atstop, t_al, t_bo, arr_par, a_par, dis_stp, im, hway, v_bus, v_pas, lpass, lapass, cap_bus,hwt,ord,count, gencount);
...
%         end
        ...
        if state(3,im) == 1 
            if imr == 3
                ...
                pds_c = floor((state(2,im)-lapass(im))*a_par(state(1,im)) + lapass(im));
%                 disp(state(2,im))
%                 disp(a_par)
%                 fprintf('pdc : %i, lapass(im) : %i a_par : %f \n', pds_c, lapass(im), a_par(state(1,im)))
                ...
                if pd_cum < pe_cum
                    fprintf('issue in split, pd_cum : %i, pe_cum : %i \n', pd_cum, pe_cum)
                end
    %             if gencount == 167
    %                 disp(load_r)
    %             end
                ...
                pa = floor(hway*arr_par(state(1,im)));
                pa_cum = pa_cum + pa;
                ...
                eps_tr = 0;
                t_nxt_i(im) = dis_stp(state(1,im))/v_bus + eps_tr + ex_wt + splittime; %As the leading module does not stop.
            ...
            elseif imr == 1
                action = 'stop';
                stcount = stcount + 1;
                pdsr = floor(lapass(im) + (state(2,im) - lapass(im))*a_par(state(1,im)));

                pd_cum = pd_cum + pdsr;
                pe_cum  = pe_cum + pdsr - lapass(im);
                if pd_cum < pe_cum
                    fprintf('issue in stop, pd_cum : %i, pe_cum : %i \n', pd_cum, pe_cum)
                end
                stmin = iminus(state(1,im),n_s);
                for iw=1:4
                    if w_pass(iw,stmin) == 0
...
%
                ...
                pw = floor((state(2,im) - lapass(im))*a_par(state(1,im))); %Number of passengers have to walk. Can modify this line to incorporate multiple skip leftover passengers
                pw_cum = pw_cum + pw;
                ...
                if ex_wt ~=0
                    ...
                    t_nxt(im) = dis_stp(state(1,im))/v_bus + eps_tr + ex_wt;
                end
            end
...
%                     hw = hw_i;
        ...
        %If join is not feasible  
                ...
                atstop(im) = 0;
                eps_tr = 0;
                tspent = dis_stp(state(1,im))/v_bus + eps_tr;  %this tspent can be the t_nxt two lines above
            end          
    end
...
fprintf('Policy cost (m) : %f \n', (w_wait*avg_wait + avg_inveh + w_walk*avg_walk1)/60)
...
legend( 'boarding','de-boarding','exiting','Awaiting','Awaiting pre','Location','northwest')
% 
figure(7)
