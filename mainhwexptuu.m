...
%     end
    ...
    %     end
        ...
        r_st = Rewardhw(im, state, 0,hw,n_s, gamma, bet, hwt);
    
        r_sk = Rewardhw(im, state, 1,hw,n_s, gamma, bet, hwt);
    
        r_sp = Rewardhw(im, state, 2,hw,n_s, gamma, bet, hwt);
       % r_st = 1; r_sk = 0;
%         fprintf('Stop rew %f \n', r_st)
%         fprintf('Split rew %f \n', r_sp)
...
%         end
        if state(3,im) == 0 && state(3,implus) == 0 %Join have positive reward and it is indeed possible to join
            r_jn = Rewardhw(im, state, 3,hw,n_s, gamma, bet, hwt);
        else
