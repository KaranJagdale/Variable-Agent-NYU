function [n_state, t_nxt, atstop, lapass, lpass, tR, tL, Hway] = nextstpredImp(im, state, act, t_nxt, atstop, hway, arr_par, a_par, unit_cap, t_al, t_bo, dis_stp, lapass, lpass, v_bus,ord,tR, tL, tSim, pos_code, actn, gencount, hw, Hway, Pa_all_cum)
...
%     disp(Hway)
    ...
    f_ind = floor(tR(state(1,im))); s_ind = floor(tSim);
    if s_ind >= size(Pa_all_cum,2)
            s_ind = size(Pa_all_cum,2)-1;
    end
    if f_ind >= size(Pa_all_cum,2)
            f_ind = size(Pa_all_cum,2)-1;
    end
...
%         end
        %pa = round(hway*arr_par(state(1,im)));
        lpass(state(1,im)) = lpass(state(1,im)) + pa;
        n_state = state;
        %n_state(1,im) = iplus(state(1,im),n_s);
        t_nxt(im) = tspent;
        tR(1,state(1,im)) = tSim;
