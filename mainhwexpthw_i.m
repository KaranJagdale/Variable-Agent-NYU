...
n_b = 4; %Number of bus, 1 bus = 2 modules
...
pb_cum = stload*n_b1; %cumulative boarding passengers
pa_cum = stload*n_b1; %cumulative arriving passengers
...
tflst = sum(dis_stp) + (t_al + t_bo)*hwt4*sum(arr_par) + fixdt*n_s;
tfstops = zeros(1,n_s);
for i = 1:n_s -1
    tfstops(i+1) = tfstops(i) + dis_stp(i) + (t_al + t_bo)*hwt4*arr_par(i) + fixdt;
end
Pa_all_cum = zeros(n_s, simtime*3600+1);
nwpasscount = 0;
for i = 1:n_s
    %passnger coming to ith stop in one target headway
    passhwt = poissrnd(arr_par(i)*hwt4);
    for k = 1:n_s-1
