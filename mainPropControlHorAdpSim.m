...
for simn=1:nsim
    ...
    %
                    ...
                    if ex_wt ~=0
                        atstop(im) = 1;
                        t_nxt(im) = ex_wt + 1; %adding 1 to ensure the module reaching late leave late
                    else
                        eps_tr = gamrnd(gam_k, gam_theta) - gam_k*gam_theta;
                        ct = dis_stp(state(1,im))/v_bus + eps_tr;
                        mxtraveltime = 0;
    ...
    costProp(simn) = (w_wait*avg_wait1 + avg_inveh + w_walk*avg_walk1)/60;
end
