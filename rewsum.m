function bestcomb = rewsum(k, state, t_nxt, atstop, t_al, t_bo, arr_par, a_par, dis_stp, im, hway, v_bus, v_pas, lpass, lapass, cap_bus,hwt,ord,count, gencount)
...
%                 end
                rew = Rewarduhwi_i(im_sim, simstate, act, a_par,arr_par, dis_stp, v_pas, ...
                hw(im_sim),cap_bus,lpass_sim(im_sim),0, lapass_sim(im_sim), hw(implus), t_bo, t_al,hw(implusp), hwt, count,v_bus, gencount, pos_code, actn);
